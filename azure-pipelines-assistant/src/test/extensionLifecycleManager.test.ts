...
suite('ExtensionLifecycleManager Tests', () => {
  ...
  setup(async () => {
    ...
    // Get fresh lifecycle manager instance
  ...
  suite('Initialization', () => {
    ...
    test('should register itself as a resource', async () => {
      await lifecycleManager.initialize(mockContext, errorHandler, mockOutputChannel);
      
      // Should have at least the lifecycle manager resource
      assert.ok(lifecycleManager.getResourceCount() >= 1);
    });
  });

  suite('Activation Lifecycle', () => {
  ...
  suite('Resource Management', () => {
    test('should register and track resources', async () => {
      ...
      assert.ok(lifecycleManager.getResourceCount() >= 2); // lifecycle manager + test resource
      assert.ok(lifecycleManager.getResourceCount('service') >= 2);
    });
  ...
  suite('State Management', () => {
    ...
    test('should emit state change events', async () => {
      ...
      const stateChanges: { oldState: ExtensionState; newState: ExtensionState }[] = [];
      lifecycleManager.on('stateChanged', (change) => {
      ...
      assert.strictEqual(stateChanges[0].oldState, ExtensionState.INACTIVE);
      assert.strictEqual(stateChanges[0].newState, ExtensionState.ACTIVATING);
      assert.strictEqual(stateChanges[1].oldState, ExtensionState.ACTIVATING);
      assert.strictEqual(stateChanges[1].newState, ExtensionState.ACTIVE);
    });
  });
});